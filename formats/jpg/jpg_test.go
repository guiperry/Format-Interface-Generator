// Code generated by FormatModule tool; DO NOT EDIT initially, but MUST be adapted.
package jpg_test // Use _test package convention
...
import (
	...
	"FormatModules/formats/jpg" // Adjust GoModulePath if needed
)

// TODO: Adapt this test function for the jpg format.
...
// TestGeneratedCode_APP0Payload tests the basic Write/Read cycle for the APP0Payload struct.
// It's a starting point and likely needs significant modification.
func TestGeneratedCode_APP0Payload(t *testing.T) {
	formatName := "jpg"
	testFilename := filepath.Join("testdata", fmt.Sprintf("test_%s_write_read.bin", formatName)) // Store test files in testdata subdir

	// --- Test Data Setup ---
	// TODO: Define meaningful sample data for jpg.APP0Payload
	// You might need data for other structs as well depending on the format.
	originalStruct := jpg.APP0Payload{
		// Field1: sampleValue1,
		// Field2: sampleValue2,
		// ... add fields based on your APP0Payload definition ...
	}
	...
	func() { // Use closure to ensure file close happens correctly
		...
		if writeErr = originalStruct.Write(writeFile /*, writeCtx */); writeErr != nil { // Comment out ctx passing
			writeErr = fmt.Errorf("error writing APP0Payload: %w", writeErr)
			return
		}
		t.Logf("-> APP0Payload written.")
	...
	func() { // Use closure for file close
		...
		t.Logf("Reading APP0Payload...")
		// Pass readCtx if needed by the Read method
		if readErr = readStruct.Read(readFile, readCtx); readErr != nil {
			readErr = fmt.Errorf("error reading APP0Payload: %w", readErr)
			return
		}
		t.Logf("-> APP0Payload read.")
	...
	if !reflect.DeepEqual(originalStruct, readStruct) {
		t.Errorf("Verification FAILED: APP0Payload structs do not match.\nOriginal: %+v\nRead:     %+v", originalStruct, readStruct)
	} else {
		t.Logf("-> APP0Payload verified.")
	}
	...
	t.Logf("Phase 3: Verification finished for APP0Payload.")
